...
3. Per-package `Description`; shared: `PackageLicenseExpression` MIT (R7), `PackageReadmeFile` (per-package or root README), `PublishRepositoryUrl`, `Microsoft.SourceLink.GitHub`, `IncludeSymbols`+`SymbolPackageFormat snupkg`, `EmbedUntrackedSources`, `Deterministic`, `ContinuousIntegrationBuild` (CI-only via env condition).
